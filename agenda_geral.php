    ...
    <a class="card<?= $sem_atividade?' muted':'' ?>" href="<?= e(APP_BASE_URL) ?>/agenda.php?mes=<?= e($competencia) ?>&funcionario_id=<?= (int)$d['id'] ?>" style="text-decoration:none;">
      <div class="spaced">
        <div style="flex:1;">
          ...
          <div class="sub muted">
            <?php if ($d['n_assin'] > 0): ?>
              <strong><?= $d['n_assin'] ?></strong> assinatura<?= $d['n_assin']>1?'s':'' ?> ·
            <?php endif; ?>
            <?php if ($d['n_entregas'] > 0): ?>
              <strong><?= $d['n_entregas'] ?></strong> entrega<?= $d['n_entregas']>1?'s':'' ?> no mês ·
              <strong><?= $d['n_clientes'] ?></strong> cliente<?= $d['n_clientes']>1?'s':'' ?>
            <?php else: ?>
              sem entregas neste mês
            <?php endif; ?>
