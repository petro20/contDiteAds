...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (($_POST['op'] ?? '') === 'salvar_capacidade_perfil') {
        if (in_array($u['role'], ['funcionario','admin','sadmin'], true)) {
            ...
            if (is_array($cap)) {
                ...
                $flash = ['ok', t('Capacidade atualizada.')];
            }
        }
    ...
    if ($nome === '') {
        $flash = ['err',t('Nome obrigatório.')];
    } elseif ($senha !== '' && strlen($senha) < 8) {
        $flash = ['err',t('Nova senha precisa ter 8+ caracteres.')];
    } else {
...
<form method="post" class="card">
  ...
  <div class="field"><label><?= e(t('Nome')) ?></label><input name="nome" required value="<?= e($u['nome']) ?>"></div>
  <div class="field"><label><?= e(t('Email')) ?></label><input value="<?= e($u['email']) ?>" disabled></div>
  <div class="field"><label><?= e(t('Perfil')) ?></label><input value="<?= e($u['role']) ?>" disabled></div>

  <?php if ($pode_editar_wisetag): ?>
    <div class="field">
      <label>WiseTag <?= empty($u['wisetag']) ? '<span style="color:var(--c-danger); font-size:11px;">⚠ ' . e(t('obrigatório pra receber pagamento')) . '</span>' : '' ?></label>
      <input name="wisetag" value="<?= e($u['wisetag'] ?? '') ?>" placeholder="@seu-wisetag">
      <div class="hint"><?= e(t('Sem WiseTag você fica fora da fila de pagamentos automáticos da Dite Ads.')) ?></div>
    </div>
    <div class="field">
      <label><?= e(t('CPF (opcional)')) ?></label>
      <input name="cpf" value="<?= e($u['cpf'] ?? '') ?>" placeholder="000.000.000-00">
    </div>
    <div class="field">
      <label><?= e(t('País (opcional)')) ?></label>
      <input name="pais" value="<?= e($u['pais'] ?? '') ?>" placeholder="<?= e(t('Brasil')) ?>">
    </div>
    <div class="field">
      <label class="check" style="font-weight:600;">
        ...
        <?= !empty($u['aceitando_clientes']) ? '🟢' : '🔴' ?> <?= e(t('Aceitando novos clientes')) ?>
      </label>
      <div class="hint"><?= e(t('Desmarque quando estiver no limite — admin vai ver alerta antes de te atribuir cliente novo.')) ?></div>
    </div>
  <?php endif; ?>

  <div class="field">
    <label><?= e(t('Nova senha (opcional)')) ?></label>
    <div class="field-password">
      <input type="password" name="senha" autocomplete="new-password" placeholder="<?= e(t('deixe em branco para manter')) ?>">
      <button type="button" class="password-toggle" onclick="togglePassword(this)" aria-label="<?= e(t('Mostrar senha')) ?>">👁</button>
    </div>
  </div>
  <button class="btn block" type="submit"><?= e(t('Salvar alterações')) ?></button>
</form>
...
<?php if (in_array($u['role'], ['funcionario','admin','sadmin'], true)):
    ...
    $cap_labels = ['criativos'=>t('Criativos (CTF/CTV/CTI)'),'postagens'=>t('Pacotes POSTAGEM'),'sites_projetos'=>t('Sites/projetos únicos')];
    $cap_map = [];
    try {
...
?>
  <h2 class="mt-5">📊 <?= e(t('Capacidade mensal declarada')) ?></h2>
  <form method="post" class="card">
    <input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>">
    <input type="hidden" name="op" value="salvar_capacidade_perfil">
    <p class="muted" style="font-size:13px;"><?= e(t('Quantos itens você consegue absorver por mês. Sistema usa pra mostrar 🟢/🔴 quando admin tenta te atribuir um cliente.')) ?> <strong><?= e(t('Mantenha atualizado.')) ?></strong></p>
...
<a class="btn btn-ghost block mt-5" href="<?= e(APP_BASE_URL) ?>/logout.php"><?= e(t('Sair')) ?></a>
