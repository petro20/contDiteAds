...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($op === 'salvar_sim') {
        ...
        if ($nome === '') {
            $flash = ['err', t('Nome é obrigatório pra salvar.')];
        } else {
            try {
                if ($sim_id) {
...
if (isset($_GET['ok'])) {
    $flash = ['ok', $_GET['ok'] === 'del' ? t('Simulação apagada.') : t('Simulação salva.')];
}
...
?>
<h1 class="page-title">📊 <?= e(t('Simulador de preço')) ?></h1>
<p class="muted"><?= e(t('Liste os custos do serviço (em USD), defina a margem desejada, e o sistema calcula o preço final pra cadastrar no catálogo. Pode salvar pra editar depois.')) ?></p>

<?php if ($flash): ?><div class="flash <?= e($flash[0]) ?>"><?= e($flash[1]) ?></div><?php endif; ?>

<?php if ($lista): ?>
<details class="card mt-3">
  <summary class="muted" style="cursor:pointer; padding:8px 0;">📂 <?= e(t('Simulações salvas')) ?> (<?= count($lista) ?>)</summary>
  <div class="mt-2">
    <?php foreach ($lista as $s): ?>
      <a class="list-card" href="?sim_id=<?= (int)$s['id'] ?>" style="<?= (int)$s['id'] === (int)$sim['id'] ? 'border-color:var(--c-primary-2);' : '' ?>">
        <div class="info">
          ...
          <div class="sub muted"><?= e(t('Atualizada')) ?> <?= e(date('d/m/Y H:i', strtotime($s['atualizado_em']))) ?></div>
        </div>
      </a>
    <?php endforeach; ?>
    <a href="<?= e(APP_BASE_URL) ?>/simulador_preco.php" class="btn btn-ghost block mt-2">+ <?= e(t('Nova simulação (em branco)')) ?></a>
  </div>
</details>
<?php endif; ?>
...
<div class="card">
  <div class="field">
    <label><?= e(t('Nome do serviço (rascunho) *')) ?></label>
    <input type="text" id="sim_nome" name="nome" value="<?= e($sim['nome']) ?>" placeholder="<?= e(t('ex: META ADS Pro, LANDING PAGE Premium')) ?>" required>
  </div>
  <div class="field">
    <label>📝 <?= e(t('Descrição do serviço')) ?></label>
    <textarea id="sim_descricao" name="descricao" rows="8" placeholder="<?= e(t("Conta em poucas palavras o que será feito. A IA vai expandir em descrição detalhada com escopo, entregas, ferramentas, responsabilidades. Ex: 'Anúncios Google Ads + Meta Ads, até 5 campanhas/plataforma'")) ?>"><?= e($sim['descricao']) ?></textarea>
    <div class="hint"><?= e(t('A IA vai DETALHAR completamente — você só dá o ponto de partida.')) ?></div>
  </div>
  <button type="button" class="btn btn-brand block" id="btn_ia" onclick="sugerirComIA()">✨ <?= e(t('Preencher com IA')) ?></button>
  <div id="ia_msg" class="hint mt-2" style="text-align:center;"></div>

  <div class="grid-2 mt-3">
    <div class="field">
      <label><?= e(t('Tipo de cobrança')) ?></label>
      <select id="sim_tipo" name="tipo" onchange="toggleSimPeriodo()">
        <option value="unico" <?= $sim['tipo']==='unico'?'selected':'' ?>><?= e(t('Único (one-shot)')) ?></option>
        <option value="mensal" <?= $sim['tipo']==='mensal'?'selected':'' ?>><?= e(t('Mensal (recorrente)')) ?></option>
        <option value="por_unidade" <?= $sim['tipo']==='por_unidade'?'selected':'' ?>><?= e(t('Por unidade')) ?></option>
      </select>
    </div>
    <div class="field" id="sim_periodo_box">
      <label><?= e(t('Período mínimo (meses)')) ?></label>
      <input type="number" id="sim_periodo" name="periodo_minimo_meses" min="0" max="60" value="<?= (int)$sim['periodo_minimo_meses'] ?>">
      <div class="hint"><?= e(t('Tempo mínimo que o cliente deve manter o contrato.')) ?></div>
    </div>
  </div>
  <label class="check"><input type="checkbox" id="sim_ia" name="tem_variante_ia" <?= $sim['tem_variante_ia']?'checked':'' ?>> <?= e(t('Tem variante "com IA" (preços alternativos)')) ?></label>
</div>
...
<h2 class="mt-5">💸 <?= e(t('Custos (USD)')) ?></h2>
<div class="card">
  <p class="muted" style="font-size:13px;"><?= t('Cada linha tem <strong>Valor total ÷ Dividir por = por unidade</strong>. Use o divisor pra rateio: ex. "Opus Clips $174 dividido por 36 vídeos" → $4.83 por vídeo. Pra custo direto, deixe divisor em 1.') ?></p>
  <p class="hint"><?= t('💡 Use o botão <strong>🔍</strong> em cada linha pra pesquisar o preço atual da ferramenta no mercado.') ?></p>

  <details class="mt-2">
    <summary class="muted" style="cursor:pointer; padding:8px 0; font-size:13px;">💼 <?= e(t('Adicionar software popular (preços referência)')) ?></summary>
    <div style="display:flex; flex-wrap:wrap; gap:6px; margin-top:8px;">
      ...
      <button type="button" class="btn btn-ghost small" onclick="adicionarLinha('Meta Business Suite', 0)"><?= e(t('Meta Suite (grátis)')) ?></button>
    </div>
  </details>

  <div id="lista_custos"></div>
  <button type="button" class="btn btn-secondary block mt-3" onclick="adicionarLinha()">+ <?= e(t('Adicionar custo')) ?></button>

  <div class="info-pair mt-3" style="border-top:1px solid var(--border); padding-top:var(--s-3);">
    <strong><?= e(t('Custo total')) ?></strong>
    <strong class="money md" style="color:var(--c-danger);">$ <span id="total_custo">0.00</span></strong>
  </div>
</div>

<h2 class="mt-5">📈 <?= e(t('Margem e preço final')) ?></h2>
<div class="card">
  <div class="grid-2">
    <div class="field">
      <label><?= e(t('Margem desejada (%)')) ?></label>
      <input type="number" id="margem" name="margem_pct" value="<?= (float)$sim['margem_pct'] ?>" min="0" step="1" oninput="recalcular()">
      <div class="hint"><?= e(t('% sobre o custo. Ex: custo $40 + 50% margem = preço $60.')) ?></div>
    </div>
    <div class="field">
      <label><?= e(t('Preço calculado (USD)')) ?></label>
      <input type="text" id="preco_calc" disabled>
    </div>
  </div>

  <div class="info-pair mt-3">
    <span class="l">💵 <?= e(t('Preço final (USD)')) ?></span>
  ...
  <div class="info-pair muted" style="font-size:13px;">
    <span class="l">↳ <?= e(t('Lucro USD')) ?></span>
    <span>$ <span id="lucro_usd">0.00</span></span>
  </div>
  <div class="info-pair muted" style="font-size:13px;">
    <span class="l">↳ <?= e(t('Em BRL (cot.')) ?> <?= number_format($cot['BRL'], 4) ?>)</span>
    <span>R$ <span id="preco_brl">0</span></span>
  </div>
  <div class="info-pair muted" style="font-size:13px;">
    <span class="l">↳ <?= e(t('Em EUR (cot.')) ?> <?= number_format($cot['EUR'], 4) ?>)</span>
    <span>€ <span id="preco_eur">0</span></span>
  </div>
</div>

<h2 class="mt-5">🤝 <?= e(t('Responsabilidades')) ?></h2>
<div class="card">
  <p class="muted" style="font-size:13px;"><?= e(t('A IA preenche esses 3 campos quando você usa "✨ Preencher com IA". Você pode editar manualmente.')) ?></p>
  <div class="field">
    <label><?= e(t('O que a Dite Ads (agência) entrega')) ?></label>
    <textarea name="resp_agencia" id="sim_resp_agencia" rows="4" placeholder="<?= e(t("• Setup das campanhas\n• Otimização semanal\n• Relatórios mensais")) ?>"><?= e($sim['resp_agencia'] ?? '') ?></textarea>
  </div>
  <div class="field">
    <label><?= e(t('O que o funcionário responsável faz')) ?></label>
    <textarea name="resp_funcionario" id="sim_resp_funcionario" rows="4" placeholder="<?= e(t("• Criação dos criativos\n• Programação das postagens\n• Acompanhamento de métricas")) ?>"><?= e($sim['resp_funcionario'] ?? '') ?></textarea>
  </div>
  <div class="field">
    <label><?= e(t('O que o cliente fornece')) ?></label>
    <textarea name="resp_cliente" id="sim_resp_cliente" rows="4" placeholder="<?= e(t("• Acesso às contas (Google Ads, Meta)\n• Briefing de público e ofertas\n• Orçamento de mídia")) ?>"><?= e($sim['resp_cliente'] ?? '') ?></textarea>
  </div>
</div>

<div class="btn-pair mt-5">
  <button type="submit" class="btn btn-secondary block" onclick="prepararSalvar()">💾 <?= e($sim['id'] ? t('Salvar alterações') : t('Salvar simulação')) ?></button>
</div>

</form>

<?php if ((int)$sim['id'] > 0): ?>
<form method="post" class="mt-3" onsubmit="return confirm('<?= e(t('Apagar esta simulação?')) ?>');">
  <input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>">
  <input type="hidden" name="op" value="apagar_sim">
  <input type="hidden" name="sim_id" value="<?= (int)$sim['id'] ?>">
  <button type="submit" class="btn btn-ghost small block" style="color:var(--c-danger);">🗑 <?= e(t('Apagar simulação')) ?></button>
</form>
<?php endif; ?>
...
<form method="get" action="<?= e(APP_BASE_URL) ?>/catalogo.php" class="mt-5">
  ...
  <button type="submit" class="btn btn-brand block" onclick="preparaCriar()">✓ <?= e(t('Criar item no catálogo com este preço')) ?> →</button>
</form>

<details class="card mt-5">
  <summary class="muted" style="cursor:pointer; padding:var(--s-3);">💡 <?= e(t('Dicas pra definir custos')) ?></summary>
  <div class="desc">
    <ul style="padding-left:20px; color:var(--txt-2);">
      <li><?= t('<strong>Pagamento ao funcionário</strong>: o que vocês acordaram em USD por execução') ?></li>
      <li><?= t('<strong>Software/licenças</strong>: rateio do custo mensal das ferramentas usadas') ?></li>
      <li><?= t('<strong>Anúncios/orçamento de mídia</strong>: se faz parte do escopo') ?></li>
      <li><?= t('<strong>Horas próprias</strong>: tempo gerencial') ?></li>
      <li><?= t('<strong>Margem realista</strong>: 40–80% pra marketing digital') ?></li>
    </ul>
  </div>
</details>
  ...
  const html = `
    <div id="${id}" style="border:1px solid var(--border); border-radius:8px; padding:10px; margin-bottom:8px;">
      <div style="display:flex; gap:6px; align-items:center; margin-bottom:6px;">
        <input type="text" class="custo-desc" placeholder="<?= e(t('ex: Pagto funcionário, Canva Pro')) ?>" value="${desc}" style="flex:1;">
        <button type="button" class="btn btn-ghost small" onclick="pesquisarPreco('${id}')" title="<?= e(t('Pesquisar preço no Google')) ?>" style="padding:6px 12px;">🔍</button>
        <button type="button" class="btn btn-ghost small" onclick="removerLinha('${id}')" title="<?= e(t('Remover')) ?>" style="padding:6px 12px;">🗑</button>
      </div>
      <div style="display:grid; grid-template-columns:1fr auto 1fr auto; gap:6px; align-items:center;">
        <div>
          <div class="hint" style="margin-bottom:2px;"><?= e(t('Valor total (USD)')) ?></div>
        ...
        <div>
          <div class="hint" style="margin-bottom:2px;"><?= e(t('Dividir por (rateio)')) ?></div>
          <input type="number" class="custo-div" placeholder="1" step="1" min="1" value="${divisor}" oninput="recalcular()">
        </div>
        <div style="text-align:right; padding-top:18px;">
          <div class="hint" style="margin-bottom:2px;"><?= e(t('= por unidade')) ?></div>
...
function pesquisarPreco(id) {
  ...
  if (!desc) { alert('<?= e(t('Preencha a descrição primeiro.')) ?>'); return; }
...
async function sugerirComIA() {
  ...
  const nome = document.getElementById('sim_nome').value.trim();
  const descricao = document.getElementById('sim_descricao').value.trim();
  if (!nome && !descricao) { msg.innerHTML = '<span style="color:var(--c-danger);"><?= e(t('Preencha o nome ou a descrição primeiro.')) ?></span>'; return; }
  btn.disabled = true;
  ...
  try {
    ...
    if (!resp.ok || !data.ok) { msg.innerHTML = '<span style="color:var(--c-danger);">' + (data.error || '<?= e(t('Erro')) ?>') + '</span>'; return; }
    ...
    recalcular();
    msg.innerHTML = '<span style="color:var(--c-success);">✓ <?= e(t('Sugestão aplicada! Revise a descrição e os custos antes de salvar.')) ?></span>';
  } catch (e) {
    msg.innerHTML = '<span style="color:var(--c-danger);"><?= e(t('Erro:')) ?> ' + e.message + '</span>';
  } finally {
    btn.disabled = false;
    btn.innerHTML = '✨ <?= e(t('Preencher com IA')) ?>';
  }
}
