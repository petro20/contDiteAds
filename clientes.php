...
if (!is_admin() && !$func_view_only) { http_response_code(403); exit(t('Acesso negado.')); }
$db = db();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!is_admin()) { http_response_code(403); exit(t('Apenas admin pode modificar.')); }
    csrf_check();
    $op = $_POST['op'] ?? '';
    ...
    if ($op === 'salvar') {
        ...
        if ($nome_emp === '') {
            $flash = ['err',t('Nome da empresa é obrigatório.')];
            ...
            if ($moeda_antiga && $moeda_antiga !== $moeda) {
                ...
                $msg_parts = [];
                if ($atualizadas) {
                    $info_cot = $moeda === 'USD' ? '' : sprintf(' (' . t('cotação USD→%s: %.4f de %s') . ')', $moeda, $cot[$moeda] ?? 0, $cot['data'] ?? '?');
                    $msg_parts[] = $atualizadas . ' ' . t('assinatura(s) com valor atualizado') . $info_cot;
                }
                if ($sem_preco)   $msg_parts[] = count($sem_preco) . ' ' . t('sem preço USD no catálogo:') . ' ' . implode(', ', $sem_preco);
    ...
    if ($op === 'apagar') {
        if (!is_sadmin()) { http_response_code(403); exit(t('Apenas Super Admin pode apagar clientes em cascata.')); }
        $pid = (int)($_POST['id'] ?? 0);
        try {
    ...
    if ($op === 'criar_login') {
        ...
        if ($cid && $email && $senha !== '') {
            try {
                ...
                $flash = ['err', (int)$e->errorInfo[1] === 1062 ? t('Já existe usuário com este email.') : t('Erro:') . ' ' . $e->getMessage()];
                $acao = 'editar'; $id = $cid;
            }
        }
...
if (isset($_GET['ok'])) {
    $msgs = ['add'=>t('Cliente criado.'),'upd'=>t('Cliente atualizado.'),'login'=>t('Login do cliente criado.')];
    $msg = $msgs[$_GET['ok']] ?? t('OK.');
    if (!empty($_GET['moeda_msg'])) $msg .= ' ' . t('Moeda alterada:') . ' ' . $_GET['moeda_msg'];
    $flash = ['ok', $msg];
}
...
if (($acao === 'novo' || $acao === 'editar') && !is_admin()) {
    http_response_code(403); exit(t('Apenas admin pode criar/editar clientes.'));
}
...
if ($acao === 'novo' || $acao === 'editar') {
    ...
    $page = $c['id'] ? t('Editar cliente') : t('Novo cliente');
    ...
    <form method="post">
      ...
      <div class="card">
        <div class="field"><label><?= e(t('Nome da empresa *')) ?></label><input name="nome_empresa" required value="<?= e($c['nome_empresa']) ?>"></div>
        <div class="field"><label><?= e(t('Nome do contato')) ?></label><input name="nome_contato" value="<?= e($c['nome_contato']) ?>"></div>
        <div class="field"><label><?= e(t('Documento (CNPJ/CPF)')) ?></label><input name="documento" value="<?= e($c['documento']) ?>"></div>
        <div class="field"><label><?= e(t('Email')) ?></label><input type="email" name="email" value="<?= e($c['email']) ?>"></div>
        <div class="field"><label><?= e(t('Moeda do cliente *')) ?></label>
          <select name="moeda" required>
            <?php foreach (['BRL'=>t('Real (R$)'),'USD'=>t('Dólar ($)'),'EUR'=>t('Euro (€)')] as $k=>$v): ?>
              <option value="<?= $k ?>" <?= $c['moeda']===$k?'selected':'' ?>><?= e($v) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
        <div class="field">
          <label><?= e(t('Dia de vencimento (1 a 31)')) ?></label>
          <input type="number" name="dia_cobranca" min="1" max="31" value="<?= $c['dia_cobranca'] !== null ? (int)$c['dia_cobranca'] : '' ?>" placeholder="<?= e(t('ex: 10')) ?>">
          <div class="hint"><?= t('Dia do mês em que este cliente paga. O sistema gera a cobrança automaticamente <strong>7 dias antes</strong> desse dia. Deixe em branco pra não gerar automático.') ?></div>
        </div>
        <div class="field"><label><?= e(t('Telefone (com DDI)')) ?></label><input name="telefone" value="<?= e($c['telefone']) ?>" placeholder="+55 11 99999-9999"></div>
        <div class="field"><label><?= e(t('Endereço')) ?></label><input name="endereco" value="<?= e($c['endereco']) ?>"></div>
        <div class="field"><label><?= e(t('Link do grupo (WhatsApp/Telegram)')) ?></label><input name="link_grupo" value="<?= e($c['link_grupo']) ?>"></div>
        <div class="field"><label><?= e(t('Observações')) ?></label><textarea name="observacoes"><?= e($c['observacoes']) ?></textarea></div>
        <label class="check"><input type="checkbox" name="ativo" <?= $c['ativo']?'checked':'' ?>> <?= e(t('Cliente ativo')) ?></label>
      </div>
      <button class="btn block" type="submit"><?= e(t('Salvar cliente')) ?></button>
    </form>

    <?php if ($c['id']): ?>
      <h2><?= e(t('Assinaturas')) ?></h2>
      <a class="btn btn-secondary block" href="<?= e(APP_BASE_URL) ?>/assinaturas.php?cliente_id=<?= (int)$c['id'] ?>"><?= e(t('Ver e atribuir itens →')) ?></a>

      <?php if (is_sadmin()): ?>
      <details class="mt-5">
        <summary class="muted" style="cursor:pointer; padding:var(--s-3);">⚠ <?= e(t('Zona de perigo')) ?></summary>
        <form method="post" class="mt-3" onsubmit="return confirm('<?= e(t('APAGAR DEFINITIVAMENTE este cliente?\n\n⚠ Vai apagar EM CASCATA tudo ligado a ele:\n  • todas as cobranças (e pagamentos recebidos)\n  • todas as assinaturas (e entregas)\n  • o login de cliente, se houver\n\nIsto NÃO pode ser desfeito. Para preservar histórico, desative em vez de apagar.\n\nConfirmar?')) ?>');">
          <input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>">
          <input type="hidden" name="op" value="apagar">
          <input type="hidden" name="id" value="<?= (int)$c['id'] ?>">
          <button class="btn btn-danger block" type="submit">🗑 <?= e(t('Apagar em cascata')) ?></button>
        </form>
      </details>
      <?php endif; ?>

      <h2><?= e(t('Acesso ao sistema')) ?></h2>
      <div class="card">
        <?php if ($userCliente): ?>
          <div class="title">✅ <?= e(t('Login ativo')) ?></div>
          <div class="desc"><?= e($userCliente['email']) ?></div>
          <p class="muted mt-3"><?= e(t('Para resetar senha do cliente, peça pra ele usar "Esqueci minha senha" no login.')) ?></p>
        <?php else: ?>
          <p class="muted"><?= e(t('Crie um login para o cliente acessar o sistema e ver as cobranças.')) ?></p>
          <form method="post" class="mt-3">
            <input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>">
            <input type="hidden" name="op" value="criar_login">
            <input type="hidden" name="cliente_id" value="<?= (int)$c['id'] ?>">
            <div class="field"><label><?= e(t('Email de login')) ?></label><input type="email" name="login_email" required value="<?= e($c['email']) ?>"></div>
            <div class="field">
              <label><?= e(t('Senha inicial')) ?></label>
            ...
            <button class="btn block" type="submit"><?= e(t('Criar login')) ?></button>
          </form>
        <?php endif; ?>
      </div>
    <?php endif; ?>
...
@keyframes tremerVenc { 0%,100%{transform:translateX(0);} 20%{transform:translateX(-2px);} 40%{transform:translateX(2px);} 60%{transform:translateX(-2px);} 80%{transform:translateX(2px);} }
.venc-pulsa { animation: pulsaVenc 1.1s ease-in-out infinite; }
.venc-tremer-azul { display:inline-block; animation: tremerVenc 0.9s ease-in-out infinite; color: var(--c-primary-2, #3B82F6); font-weight:600; }
</style>
...
<?php foreach ($clientes as $cl): ?>
  <?php if ($func_view_only): ?>
    <a class="list-card" href="<?= e(APP_BASE_URL) ?>/agenda.php?cliente_id=<?= (int)$cl['id'] ?>">
      <div class="info">
        <div class="nome"><?= e($cl['nome_empresa'] ?: t('(sem nome)')) ?></div>
        <div class="sub"><?= e($cl['nome_contato'] ?? '—') ?> · <?= e($cl['moeda']) ?> · <?= (int)$cl['qtd_assin'] ?> <?= $cl['qtd_assin']==1?e(t('serviço')):e(t('serviços')) ?></div>
      </div>
    </a>
  <?php else: ?>
    ...
    <a class="list-card" href="?acao=editar&id=<?= (int)$cl['id'] ?>">
      <div class="info">
        <div class="nome">
          <?= e($cl['nome_empresa'] ?: t('(sem nome)')) ?>
          <?php if (!$cl['ativo']): ?><span class="status status-info"><?= e(t('inativo')) ?></span><?php endif; ?>
        </div>
        <div class="sub">
          ...
          <?php elseif (!empty($cl['dia_cobranca'])): ?>
            · <span class="venc-tremer-azul"><?= e(t('vence dia')) ?> <?= (int)$cl['dia_cobranca'] ?></span>
          <?php elseif ($ac['qtd'] > 0): ?>
...
<?php endforeach; ?>
<?php if (!$clientes): ?>
  <p class="muted center mt-5"><?= $func_view_only ? e(t('Você ainda não tem clientes atribuídos.')) : e(t('Nenhum cliente. Use "+ Novo" ou "Convidar".')) ?></p>
<?php endif; ?>
